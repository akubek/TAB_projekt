...
ON DELETE CASCADE,
    ...
    price_modifierDECIMAL(
        10,
        2
    ) DEFAULT 0.00,
    stock_quantity INT NOT NULL DEFAULT 0CHECK(stock_quantity >= 0)
);
...
ON DELETE CASCADE,
    rating INT NOT NULL CHECK(rating BETWEEN 1
    AND 5),
    comment TEXT,
...
CREATE TABLE orders(
    ...
    user_id INT NOT NULL REFERENCES users(id)
ON DELETE RESTRICT,
    statusVARCHAR(50) NOT NULL DEFAULT 'NEW',
    total_priceDECIMAL(
        10,
        2
    ) NOT NULL,
    applied_promotion_id INT REFERENCES promotions(id)
ON DELETE 
SET NULL,
shipping_address JSONB NOT NULL,
delivery_methodVARCHAR(100) NOT NULL,
payment_methodVARCHAR(100) NOT NULL,
wants_invoice BOOLEAN NOT NULL DEFAULT FALSE,
invoice_company_nameVARCHAR(255),
invoice_nipVARCHAR(50),
